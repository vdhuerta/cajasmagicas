...
const UserDetailsModal: React.FC<UserDetailsModalProps> = ({ user, onClose }) => {
    ...
    useEffect(() => {
        const loadLogs = async () => {
            ...
            setError('');
            try {
                const userLogs = await adminService.getUserLogs(user.id);
                setLogs(userLogs);
            } catch (err: any) {
                console.error("Failed to fetch user logs via admin service:", err);
                setError(`No se pudieron cargar los registros de desempeño: ${err.message}.`);
            } finally {
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <div className="flex justify-center gap-4 mt-2 text-xs font-semibold">
                                    <div className="flex items-center gap-2"><div className="w-3 h-3 bg-sky-500/50 border border-sky-700"></div><span className="text-sky-800">Clasificación y Afines</span></div>
                                    <div className="flex items-center gap-2"><div className="w-3 h-3 bg-rose-500/50 border border-rose-700"></div><span className="text-rose-800">Habilidades de Seriación</span></div>
                                </div>
                            </div>
