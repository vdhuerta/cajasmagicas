...
const ClearDataConfirmationModal: React.FC<ClearDataConfirmationModalProps> = ({ onConfirm, onCancel }) => {
  const title = "¡Acción Irreversible!";
  const message = "Estás a punto de borrar todos tus datos de progreso, incluyendo los niveles completados y los logros desbloqueados. Esta acción no se puede deshacer. ¿Estás seguro de que quieres empezar de cero?";
    ...
    <div className="fixed inset-0 bg-slate-800 bg-opacity-60 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
      <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 w-full max-w-md relative animate-fade-in-up text-center">
        ...
        <div className="flex justify-center gap-4">
            ...
            <button 
              onClick={onConfirm} 
              className="w-full px-6 py-3 bg-red-600 text-white font-bold text-lg rounded-xl shadow-lg hover:bg-red-700 transition"
            >
              Sí, Limpiar
            </button>
        </div>
